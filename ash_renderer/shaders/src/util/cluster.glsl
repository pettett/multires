...
float get_this_error(uint cluster_index, vec3 local_cam_pos){
    ...
    if (co_parent >= 0) {
        ...
        this_error = min(
			...
			cluster_error(uint(co_parent), local_cam_pos)
			);
		#else
        this_error = max(
			cluster_error(cluster_index,   local_cam_pos), 
...
// 					clusters[cluster_index].parent1 == -1 ? cluster_index : clusters[cluster_index].parent1  // Error is higher then ours, only need to draw the parent (parent1 > parent0)
// 				)
// 			) ;		
